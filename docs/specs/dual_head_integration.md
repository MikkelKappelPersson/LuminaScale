# Spec: ACES Mapper & Dual-Head Integration

## 1. Overview
The ACES mapper is the second major component of the LuminaScale pipeline. While the Dequantization-Net (Head 1) focuses on structural restoration and bit-depth expansion, the ACES mapper (Head 2) performs "blind" color space normalization to the ACES (Academy Color Encoding System) workspace.
...
*   **Execution**: Two separate training scripts and trainers sharing the same WebDataset pipeline.
    *   `scripts/train_dequant_net.py` -> `DequantizationTrainer`
    *   `scripts/train_mapper.py` -> `MapperTrainer`
...
*   **Data Flow**:
    ...
    2.  `Restored 32-bit` -> `ACES Mapper` -> `Final ACES2065-1 output`
...
3.  **`AcesMapperTrainer`**: Inherits from `BaseLuminaScaleTrainer`. Focused on color statistics and "blind" perceptual losses.
...
│   ├── dequant_dev.yaml       
│   ├── mapper.yaml          # Phase 1: ACES specific params
│   ├── mapper_dev.yaml          
│   └── joint.yaml         # Phase 2: Joint weights/LRs
│   └── joint_dev.yaml         
└── config.yaml         # Master config (Composition)
```

*   **Task Switching**: Using `python scripts/train.py task=joint` will automatically compose the required heads.
*   **Checkpoint Overrides**: The `joint.yaml` will contain paths to pre-trained checkpionts for initialization (`dequant_pretrained_path`, `mapper_pretrained_path`).
...
*   **Directory Scoping**:
    *   `outputs/dequant_runs/training/` -> (Pre-training Head 1)
    *   `outputs/mapper_runs/training/`   -> (Pre-training Head 2)
    *   `outputs/joint_runs/training/`  -> (Joint Fine-tuning Phase)

### Multi-Instance Visualization:
*   **Execution**: Each task creates a completely independent TensorBoard logger instance.
*   **User Interface**: Users can view the tasks individually or compare them by pointing TensorBoard at the root `outputs/task/training/` folder, which will list all three tasks as distinct "runs" in the sidebar.
