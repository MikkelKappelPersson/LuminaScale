...
class LuminaScaleModule(L.LightningModule):
    ...
    def _process_batch(self, batch: tuple[list[bytes], list[dict]]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if is_repeated_image:
            ...
            logger.info(f"[CACHE HIT] Image '{current_image_id}' (Hit rate: {hit_rate:.1f}% | {self._cache_hits} hits / {self._total_evals} evals)")
            ...
            hit_rate = (self._cache_hits / self._total_evals * 100) if self._total_evals > 0 else 0
            logger.info(f"[CACHE MISS] Image '{current_image_id}' (Hit rate: {hit_rate:.1f}% | {self._cache_hits} hits / {self._total_evals} evals)")
            t_start = time.perf_counter()
            ...
            if current_image_id and len(exr_bytes_list) == 1:
                ...
                logger.info(f"[CACHE STORE] Cached image '{current_image_id}' for reuse (cache size: {srgb_8u_batch.element_size() * srgb_8u_batch.nelement() / 1e6:.1f}MB)")
    ...
    def training_step(self, batch: tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> torch.Tensor:
        """Process one image with 32 random crops and update gradients after EACH crop.
        
        Strategy (32 gradient updates per image):
        1. Decode image once (batch_size=1 image from WebDataset)
        2. Cache the decoded full image
        3. For each of 32 random crops:
           a. Generate random crop from cached image
           b. Forward pass through model
           c. Compute loss
           d. Backward pass (accumulate gradients)
           e. Optimizer step (UPDATE PARAMETERS)
           f. Zero gradients
        4. Release cache, move to next image
        
        Result: Each image decoded once, 32 gradient updates per image (32x faster learning).
        """
        try:
            ...
            if isinstance(batch, (list, tuple)):
                if len(batch) >= 2:
                    # LMDB dataset: direct tensor batch
                    ...
                    # WebDataset case: batch is (exr_bytes_list, metadata_list)
                        ...
                        # Step 1: Decode the image ONCE
                        x_full, y_full = self._process_batch(batch)  # [1, 3, H, W] batched
                        
                        # Unbatch to [3, H, W]
                        if x_full.ndim == 4:
                            x_full = x_full[0]  # [1, 3, H, W] -> [3, H, W]
                            y_full = y_full[0]
                        
                        logger.info(
                            f"[BATCH {batch_idx}] Decoded image once, generating {num_patches_per_image} crops..."
                        )
                        
                        # Step 3: Generate and train on num_patches_per_image crops
                        # Each crop gets its own backward pass and optimizer step
                        crop_losses = []
                        zero_loss_count = 0
                        
                        for patch_idx in range(num_patches_per_image):
                            # Step 3a: Generate one random crop from full image [3, H, W]
                            C, H, W = x_full.shape
                            crop_size = 512
                            
                            if H <= crop_size or W <= crop_size:
                                # Image smaller than crop - use full
                                x_crop = x_full
                                y_crop = y_full
                            else:
                                # Random crop from full image [3, H, W]
                                top = torch.randint(0, H - crop_size + 1, (1,)).item()
                                left = torch.randint(0, W - crop_size + 1, (1,)).item()
                                x_crop = x_full[:, top:top+crop_size, left:left+crop_size]
                                y_crop = y_full[:, top:top+crop_size, left:left+crop_size]
                            
                            # Add batch dimension for model [1, 3, H, W]
                            x_crop = x_crop.unsqueeze(0)
                            y_crop = y_crop.unsqueeze(0)
                            
                            # Convert to float32 if needed (raw WebDataset data is uint8)
                            ...
                            # Step 3c: Compute loss for this crop  
                            mask = exposure_mask(y_crop)
                            crop_loss = masked_l2_loss(y_hat, y_crop, mask)
                            crop_loss_val = crop_loss.item()
                            crop_losses.append(crop_loss_val)
                            
                            # DEBUG: First crop of first batch only
                            if batch_idx == 0 and patch_idx == 0:
                                mask_sum = mask.sum().item()
                                mask_size = mask.numel()
                                mask_ratio = mask_sum / mask_size if mask_size > 0 else 0
                                logger.info(
                                    f"[DEBUG CROP 0] y_crop dtype={y_crop.dtype}, shape={y_crop.shape}, "
                                    f"y_hat dtype={y_hat.dtype}, shape={y_hat.shape} | "
                                    f"mask: {mask_sum:.0f}/{mask_size} pixels active ({mask_ratio*100:.1f}%) | "
                                    f"loss={crop_loss_val:.4e}"
                                )
                            
                            if crop_loss_val == 0.0:
                                zero_loss_count += 1
                            ...
                            # Step 3e: UPDATE GRADIENTS (optimizer step after each crop)
                            optimizer.step()
                        
                        # Calculate average loss for logging
                        avg_loss = sum(crop_losses) / num_patches_per_image
                        
                        self.last_batch_gpu_ms = (time.perf_counter() - t_start) * 1000
                        self.last_batch_loss = avg_loss
                        
                        logger.info(
                            f"[BATCH {batch_idx}] ✓ Processed 32 patches (1 image) with 32 gradient updates. "
                            f"Avg loss: {avg_loss:.4e}, GPU time: {self.last_batch_gpu_ms:.1f}ms "
